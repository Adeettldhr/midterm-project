...
public class LibraryBranchController : Controller
{
    ...
    public async Task<IActionResult> Details(int? id)
    {
        ...
        var branch = await _context.LibraryBranches.FirstOrDefaultAsync(m => m.LibraryBranchId == id);
    ...
    public async Task<IActionResult> Create([Bind("Name,Location")] LibraryBranch branch)
    {
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, [Bind("LibraryBranchId,Name,Location")] LibraryBranch branch)
    {
        if (id != branch.LibraryBranchId) return NotFound();
        if (ModelState.IsValid)
        {
            try { _context.Update(branch); await _context.SaveChangesAsync(); }
