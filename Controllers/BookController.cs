...
public class BookController : Controller
{
    ...
    public async Task<IActionResult> Create([Bind("Title,AuthorId,ISBN,PublishedYear,Genre")] Book book)
    {
    ...
    // POST: Book/Edit/5
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, [Bind("BookId,Title,AuthorId,ISBN,PublishedYear,Genre")] Book book)
    {
        if (id != book.BookId) return NotFound();
        if (ModelState.IsValid)
        {
            try { _context.Update(book); await _context.SaveChangesAsync(); }
